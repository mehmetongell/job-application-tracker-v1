...
import { 
  LayoutDashboard, Briefcase, User, LogOut, PlusCircle, Search, Bell, MapPin, X, Calendar, Mail, Shield
} from 'lucide-react';
import API from './api';
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-xl">
            <div className="flex items-center gap-6 mb-10">
              <div className="w-24 h-24 bg-indigo-600 rounded-3xl flex items-center justify-center text-white text-3xl font-bold shadow-xl shadow-indigo-100">
                <User size={40} />
              </div>
              <div>
                <h2 className="text-2xl font-bold text-slate-900">Mehmet Öngel</h2>
                <p className="text-slate-500 font-medium">Full Stack Developer Adayı</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">E-posta</p><p className="font-bold text-slate-700">mehmet@example.com</p></div>
              </div>
              <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-emerald-600 shadow-sm"><Shield size={18}/></div>
                <div><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Hesap Durumu</p><p className="font-bold text-slate-700">Doğrulanmış Geliştirici</p></div>
              </div>
            </div>
            
            <button className="w-full mt-10 bg-slate-900 text-white py-4 rounded-2xl font-bold hover:bg-black transition-all">
              Bilgileri Güncelle
            </button>
          </div>
        </div>
  ...
  return (
    <aside className="w-72 bg-white border-r border-slate-200 flex flex-col sticky top-0 h-screen">
      <div className="p-8 flex items-center gap-3">
        <div className="h-10 w-10 bg-indigo-600 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200"><Briefcase className="text-white" size={22} /></div>
      ...
      <main className="flex-1 p-8">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-md rounded-[32px] p-8 animate-in zoom-in duration-200 border border-white/20 shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all">Ekle</button>
              </form>
            </div>
          </div>
        )}
        ...
        <header className="flex justify-between items-center mb-10">
          <div className="relative w-96">
            ...
            <input type="text" placeholder="Hangi başvuruyu aramıştın?" className="w-full pl-12 pr-4 py-3.5 bg-white border border-slate-200 rounded-2xl outline-none shadow-sm focus:ring-2 ring-indigo-500/10 transition-all font-medium" value={search} onChange={(e) => { setSearch(e.target.value); fetchData(e.target.value); }} />
          </div>
          <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-3.5 rounded-2xl font-bold shadow-xl shadow-indigo-200 transition-all hover:-translate-y-0.5 active:translate-y-0 active:shadow-none"><PlusCircle size={20} /><span>Yeni Başvuru</span></button>
        </header>
        ...
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-xl overflow-hidden">
          <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/30">
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Aktif Başvurular</h2>
            <div className="flex items-center gap-2">
               <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span>
               <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">{jobs.length} Toplam Kayıt</span>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead><tr className="text-slate-400 text-[10px] uppercase tracking-widest font-black border-b border-slate-50"><th className="px-8 py-5">Şirket & Pozisyon</th><th className="px-8 py-5 text-center">Durum</th><th className="px-8 py-5">Konum</th><th className="px-8 py-5 text-right">Tarih</th></tr></thead>
              <tbody className="divide-y divide-slate-50">
                {jobs.map((job) => (
                  <tr key={job.id} className="group hover:bg-indigo-50/30 transition-all">
                    <td className="px-8 py-6"><div className="flex flex-col"><span className="font-bold text-slate-900 text-lg group-hover:text-indigo-600 transition-colors">{job.company}</span><span className="text-slate-400 text-sm font-medium">{job.position}</span></div></td>
